...
Respect what the person says about themselves under "About me" and any dislikes in a friend's facts — never suggest something either has said they don't like.
...
export const FEEDBACK_SYSTEM = `Someone is reacting to a suggested outing. Turn what they wrote into one or more short memories, each about either themselves ("me") or one named friend ("friend").
Return JSON {"memories":[{"about":"me"|"friend","name":"friend's first name or empty","memory":"short third-person-free statement, e.g. doesn't like comedy / prefers daytime plans / can't do weekday evenings"}]}.
Only use friend names from the list you are given. "I", "me", "we" refer to the writer → about "me". If the text is only a complaint about the venue with nothing to remember, return {"memories":[]}. Keep each memory under 12 words.`;
